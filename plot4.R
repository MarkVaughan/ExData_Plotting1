...
# specifies to construct 4 graphs over 2 rows and 2 two columns
par(mfrow=c(2,2))

plot(dmy_hms(paste(hpc$Date,hpc$Time)),hpc$Global_active_power,type="l",xlab="",ylab="Global Active Power")

plot(dmy_hms(paste(hpc$Date,hpc$Time)),hpc$Voltage,type="l",ylab="Voltage",xlab="datetime")
...
legend("topright",col = c("black","blue","red"),lty =c(1,1,1),bty = "n", c("Sub_metering_1","Sub_metering_2","Sub_metering_3"))

plot(dmy_hms(paste(hpc$Date,hpc$Time)),hpc$Global_reactive_power,type="l",ylab="Global_reactive_power",xlab="datetime")
